...
## Supply chain (CI gates — `.github/workflows/ci.yml`)

Baseline: ADR 0044, **as amended for the secret scan by ADR 1028** (pinned CLI
binary + sha256 digest, full-history scan on every event). ADR 0044's own
`gitleaks` bullet still describes the superseded `gitleaks-action` arrangement
and is marked partially superseded — cite 1028, not 0044, for anything about
the secret-scan gate. The table below is the as-built description.

| Gate                  | Where                                                                                                                                                                                                                                                            | Policy                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       | Allowlist mechanism                                                                                                       |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------- |
| Secret scan           | `gitleaks` job — a **pinned CLI binary** (version _and_ sha256 digest, ADR 1028; `gitleaks-action` is no longer used), plus lefthook pre-commit on the staged diff (skips gracefully when the binary is absent). `pnpm check:gitleaks-pin` guards the pin itself | **Full history on every run.** The job checks out at `fetch-depth: 0` and runs `gitleaks detect --source .`, so every event that starts CI scans every commit — there is no incoming-range mode. In THIS repo those events are `pull_request`, `schedule` and `workflow_dispatch`: there is deliberately no `push` trigger (see the trigger-policy header in `ci.yml`), so a bare push to `main` runs no gitleaks job at all — the pre-commit hook is the only secret-scan gate on that path | `.gitleaks.toml` — test-fixture **paths** only, never a real leak                                                         |
