...
console.log(`  ✔ apps/api/.env.local — BETTER_AUTH_SECRET + Centrifugo secrets`);
...
# See apps/web/.env.example for every available variable. Uncomment API_URL
# to leave mock mode and target the local api:
# API_URL=http://localhost:4000
`,
  flags.force,
);
console.log(`  ✔ apps/web/.env.local — scaffolded (no server secrets by design)`);

// Compose project name: docker/compose.yaml hardcodes `name:
// fullstack-skeleton`, so two stamped repos (or a stamp + the skeleton
// itself) on one machine would otherwise share/recreate each other's
// containers. COMPOSE_PROJECT_NAME in the gitignored docker/.env takes
// precedence over the file-level `name:` (compose project-name precedence:
// -p flag > env var > name: > directory), namespacing the stack per project.
writeEnvFile(
  join(repoRoot, "docker/.env"),
  `# Generated by \`node scripts/create-project\` on ${createdAt}.
# Gitignored. Namespaces this project's dev containers so they never collide
# with other repos stamped from the same skeleton (compose.yaml hardcodes
# \`name: fullstack-skeleton\`). Host-port overrides can live here too
# (POSTGRES_HOST_PORT, REDIS_HOST_PORT — see docker/compose.yaml).
COMPOSE_PROJECT_NAME=${name}
`,
  flags.force,
);
console.log(`  ✔ docker/.env — COMPOSE_PROJECT_NAME=${name} (container namespace)`);
