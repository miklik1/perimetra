...
// Compose project name + host-port remap: docker/compose.yaml defaults its
// project name to `fullstack-skeleton` AND every host port (5432/6379/8000/
...
// per-project offset. The compose `name:` default is ALSO rewritten below so
// the namespace survives even when this gitignored docker/.env is absent (a
// fresh clone on another machine) — the collision that let a derived project
// share the skeleton's volumes.
writeEnvFile(
...
// Bake the project name into the COMMITTED compose file's `name:` default, so
// the Docker volume/network namespace is per-project even on a fresh clone with
// no docker/.env (which is gitignored). The env var still wins when present;
// this is the fail-safe default. Guarded: if the anchor line has drifted, fail
// loudly rather than silently leave every stamp sharing `fullstack-skeleton`.
const composePath = join(repoRoot, "docker/compose.yaml");
const composeSrc = readFileSync(composePath, "utf8");
// Match the current `:-default` token whatever it is (skeleton `fullstack-skeleton`
// on a fresh clone, or a prior name on a --force re-stamp) so re-stamping works.
const composeNameRe = /name: \$\{COMPOSE_PROJECT_NAME:-[^}]*\}/;
if (!composeNameRe.test(composeSrc)) {
  fail(
    `docker/compose.yaml is missing the expected project-name line\n` +
      `  name: \${COMPOSE_PROJECT_NAME:-<default>}\n` +
      `The template drifted — update scripts/create-project to match before stamping,\n` +
      `or two stamped repos will share Docker volumes (ADR 1014).`,
  );
}
writeFileSync(
  composePath,
  composeSrc.replace(composeNameRe, `name: \${COMPOSE_PROJECT_NAME:-${name}}`),
);
console.log(`  ✔ docker/compose.yaml — project name default → ${name} (volume/network namespace)`);
