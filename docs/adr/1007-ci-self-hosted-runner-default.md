...
- **Open verification (CHECK 2026-07-19):** whether a payment-failed account state _also_ refuses to schedule _self-hosted_ jobs. Self-hosted minutes are unbilled, but a bad account state can refuse to schedule any job; the `workflow_dispatch` ~6 s deaths suggest it might. Confirm on the first live runner run; if it gates self-hosted too, the account payment fix (Martin, account-level) is the hard unblock.
