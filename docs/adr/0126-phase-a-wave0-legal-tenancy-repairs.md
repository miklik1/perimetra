...
> **SUPERSEDED IN PART, 2026-07-28 — the shape-only rule was itself a leak (skeleton [ADR 1035](1035-the-error-envelope-declared-details-and-never-sent-it.md), drained in W16).** "Forwards a plain-object `details`" is no longer how the filter behaves, and the reason is that the shape rule cannot tell a payload a service chose to publish from one a third-party library happened to park under the same key. `@nestjs/terminus` throws a body whose own `details` is the readiness map, so this repo's `GET /health/ready` — `@Public()` and `@SkipThrottle()` — was publishing which internal dependencies exist and which one is failing to any anonymous caller. (Narrower here than it could have been: [ADR 0099](0099-authz-default-deny-app-guard.md) had already reduced each indicator to `{ message: "unreachable" }`, so what shipped was per-dependency up/down rather than a DSN or a driver internal.) `details` is now **opt-in**: the filter publishes it only from a body built by the exported `errorEnvelope()` helper, which stamps an unforgeable module-local `Symbol`, and both runtime guards are derived from `apiErrorEnvelopeSchema` rather than restating it. The rest of this section still holds — one slot, context thrown anywhere else is still dropped — but a bare `{ message, code, details }` throw now loses its `details` silently, so every typed rejection must be built with `errorEnvelope()`. All three of this repo's call sites (`order_exists_for_chain`, `quote_superseded`, `site_invalid`) were rewritten in the same drain.
