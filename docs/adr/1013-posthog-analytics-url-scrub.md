...
- **Fixed upstream (channel A) — the divergence is closed.** The two defects are skeleton `2f83b8d`'s own, not artefacts of this port, so they were live upstream and in every repo stamped from it that had drained that commit. The correction is now upstream as skeleton **`7dc86db` (ADR 1015)**, and `skeleton.baseCommit` is bumped to it — perimetra and the skeleton now agree, so this is no longer a divergence to carry. **Still exposed as of 2026-07-19** and needing the same correction: `booking` (drained `2f83b8d`, leak baked in) and `web-native-skeleton` (an independent lineage that relocated the regex into `packages/utils/src/pii.ts`). `anyora-platform`, `mercata` and `primat-plus-app` sit on an older `baseCommit` and are safe only by not carrying the feature yet — they must not drain forward past `2f83b8d` without picking up `7dc86db`. The reusable bug class is written up in the vault as _A redaction tail narrowed to spare its carrier under-redacts — a security regex must fail toward over-redaction_.
