/**
 * Repository unit test — proves every query goes through the org `scoped()`
 * seam (ADR 0055) and the keyset cursor math, against a chainable drizzle mock
 * (real SQL objects rendered for assertions).
 */
 ...
 * repository builds (real drizzle SQL objects — rendered for assertions) and
 * resolves with the given rows.
 */
function makeTxHost(rows: unknown[] = []) {
  ...
  type TxHost = ConstructorParameters<typeof ReleaseDraftsRepository>[0];
  return { txHost: { tx } as unknown as TxHost, captured };
}

const row = (id: string) => ({ id, organizationId: SCOPE.organizationId });

describe("ReleaseDraftsRepository scope filtering (ADR 0055)", () => {
  it("list filters by organization and excludes soft-deleted rows", async () => {
    const { txHost, captured } = makeTxHost([]);
    await new ReleaseDraftsRepository(txHost).list(SCOPE, { limit: 20, sort: "createdAt:desc" });
  ...
  it("findById, update and softDelete all carry the org scope filter", async () => {
    for (const exercise of [
      (repo: ReleaseDraftsRepository) => repo.findById(SCOPE, ID_1),
      (repo: ReleaseDraftsRepository) => repo.update(SCOPE, ID_1, { modelId: "x" }),
      (repo: ReleaseDraftsRepository) => repo.softDelete(SCOPE, ID_1),
    ]) {
      const { txHost, captured } = makeTxHost([]);
      await exercise(new ReleaseDraftsRepository(txHost));
  ...
  it("insert stamps ownerId (audit) and organizationId (scope) plus the domain fields", async () => {
    const { txHost, captured } = makeTxHost([row(ID_1)]);
    await new ReleaseDraftsRepository(txHost).insert(SCOPE, {
      modelId: "sliding-gate",
      version: 2,
      catalogVersion: 2,
      baseReleaseId: "sliding-gate@1",
      body: { step: 1 },
    });
...
describe("ReleaseDraftsRepository keyset pagination", () => {
  it("fetches limit+1 and turns the extra row into nextCursor", async () => {
    ...
    const page = await new ReleaseDraftsRepository(txHost).list(SCOPE, {
      limit: 2,
      sort: "createdAt:desc",
    });
